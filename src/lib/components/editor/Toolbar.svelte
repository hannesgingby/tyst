<script lang="ts">
    ...
    const referenceForceOpen = $derived(referenceTied && documentStore.popupDismissed !== "reference");
            ...
            >
                ...
                {:else if groupIndex === 4}
                    ...
                    <HoverPopup
                        ...
                        icon="at-sign"
                        iconClass="size-5"
                        active={referenceTied}
                        forceOpen={referenceForceOpen}
                    >
                        {#snippet popup()}
                            <ReferencePopup
                                activeTargetId={activeBlock.reference?.targetBlockId ?? activeBlock.citation?.sourceId ?? null}
                                isCitationActive={!!activeBlock.citation}
                            />
                        {/snippet}
                    </HoverPopup>
                    {#each group.tools as tool, toolIndex (toolIndex)}
