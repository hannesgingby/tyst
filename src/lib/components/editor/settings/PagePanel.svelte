<script lang="ts">
	...
	$effect(() => {
		if (activeIndex > pageCount - 1) documentStore.setActivePage(pageCount - 1);
	});
...
{#snippet sectionHeader(title: string, section: PageSection)}
	<div class="mb-5 flex items-center justify-between border-b border-bg-600 pb-3">
...
<div class="flex flex-col gap-1">
				...
				{#each Array(pageCount) as _, index (index)}
					<button
						...
						class={[
							...
							index === activeIndex ? "bg-bg-850 text-text-100" : "text-text-100 hover:bg-bg-850",
						]}
	...
	<p class="text-body-16 text-text-200">Choose page dimensions, margins and more</p>
</div>
	...
	<div class="flex flex-col gap-[18px]">
		<div class="flex items-center justify-between gap-4">
			...
			<DropdownMenu
				...
				onchange={(value: PaperPreset) => documentStore.setPaperPreset(value)}
			/>
		</div>
			...
			<div class="flex items-center gap-3">
				<div class="flex items-center gap-1.5">
					...
					<Input
						...
						step={1}
						disabled={!isCustom}
						onchange={(value) => documentStore.setDimension("width", value)}
					/>
				</div>
				<div class="flex items-center gap-1.5">
					...
					<Input
						...
						step={1}
						disabled={!isCustom}
						onchange={(value) => documentStore.setDimension("height", value)}
					/>
				</div>
			</div>
		</div>
	...
	<div class="flex gap-12">
		...
		<div class="flex w-[252px] flex-col gap-2.5">
			<div class="flex items-center justify-end gap-2">
				...
				<Input class="w-[151px]" bg={FIELD_BG} bind:value={marginSrc.margins.x} unit="pt" emptyLabel="Auto" decimals={0} min={0} max={2000} step={1} />
			</div>
			<div class="flex items-center justify-end gap-2">
				<span class="w-12 text-right text-body-14 text-text-100">Y</span>
				<Input class="w-[151px]" bg={FIELD_BG} bind:value={marginSrc.margins.y} unit="pt" emptyLabel="Auto" decimals={0} min={0} max={2000} step={1} />
			</div>

			<div class="mt-4 flex flex-col gap-2.5">
				<div class="flex items-center justify-end gap-2">
					<span class="w-12 text-right text-body-14 text-text-100">Left</span>
					<Input class="w-[151px]" bg={FIELD_BG} bind:value={marginSrc.margins.left} unit="cm" decimals={1} min={0} max={20} step={0.1} />
				</div>
				<div class="flex items-center justify-end gap-2">
					<span class="w-12 text-right text-body-14 text-text-100">Right</span>
					<Input class="w-[151px]" bg={FIELD_BG} bind:value={marginSrc.margins.right} unit="cm" decimals={1} min={0} max={20} step={0.1} />
				</div>
				<div class="flex items-center justify-end gap-2">
					<span class="w-12 text-right text-body-14 text-text-100">Top</span>
					<Input class="w-[151px]" bg={FIELD_BG} bind:value={marginSrc.margins.top} unit="cm" decimals={1} min={0} max={20} step={0.1} />
				</div>
				<div class="flex items-center justify-end gap-2">
					<span class="w-12 text-right text-body-14 text-text-100">Bottom</span>
					<Input class="w-[151px]" bg={FIELD_BG} bind:value={marginSrc.margins.bottom} unit="cm" decimals={1} min={0} max={20} step={0.1} />
				</div>
			</div>
		</div>
	</div>
		...
		<div class="flex items-start justify-between gap-4">
			<div class="flex flex-col gap-0.5">
				...
				<span class="max-w-[360px] text-body-12 text-text-200">
					This will be overwritten if an explicit color for some text is set.
				</span>
			</div>
