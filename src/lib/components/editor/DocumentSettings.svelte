<script lang="ts">
	...
	type SectionId = "metadata" | "page" | "references" | "format-groups" | "preferences";
	...
	const navGroups: { heading: string; items: NavItem[] }[] = [
		{
			...
			items: [
				...
				{ id: "format-groups", label: "Format groups", icon: "text-size" },
			],
		},
		{
			heading: "Editor",
			items: [{ id: "preferences", label: "Preferences", icon: "preferences" }],
		},
	];
...
{#if open}
	...
	>
		<!-- svelte-ignore a11y_click_events_have_key_events, a11y_no_static_element_interactions -->
		<div
			class="flex h-full max-h-[826px] w-full max-w-[1120px] overflow-hidden rounded-xl border border-bg-700 bg-white shadow-[0_4px_40px_0_rgba(0,0,0,0.12)]"
			role="dialog"
			aria-modal="true"
		...
		>
			<!-- Sidebar -->
			<nav class="flex w-[228px] shrink-0 flex-col gap-5 bg-bg-950 p-3">
				{#each navGroups as group (group.heading)}
					<div class="flex flex-col gap-1">
						<span class="px-2 pb-1 text-body-12 text-text-250">{group.heading}</span>
			...
			<!-- Content -->
			<div class="relative min-w-0 flex-1 overflow-y-auto px-12 py-10">
				<button
					type="button"
				...
				<div class="mx-auto max-w-[702px]">
					{#if active === "page"}
						<PagePanel />
					{:else}
						<div class="flex h-64 items-center justify-center text-body-14 text-text-200">
							Coming soon.
						</div>
					{/if}
				</div>
